...
脚本会读取 `通州校区建筑/已有建筑实景/` 中有实景资料的建筑，并读取 `通州校区建筑/未有建筑设计图/` 中的未来建筑。15 个大地图坐标维护在脚本的 `POSITION_OVERRIDES` 中，未来建筑会生成 `mapEnabled: false`。
