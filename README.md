...
- 校园地图导览
  ...
  - 已按手绘地图中的建筑名称和红色虚线指示重新校准 15 个地图建筑坐标
  - `中心食堂`、`先锋剧场`、`未来传播中心` 作为“敬请期待”地点展示，不进入大地图索引
  - 地图支持放大、缩小、复位和拖拽浏览，移动端可更精细查看局部区域

- 校园建筑索引
  - 读取 `通州校区建筑/已有建筑实景/` 中的建筑图片
  - 读取 `通州校区建筑/未有建筑设计图/` 中的建筑设计图，并归入“敬请期待”板块
  - 读取各建筑 `简介/cn|en|es|fr|ge|ja|ru.txt` 多语种文本
  - 地图地点和敬请期待地点在索引中分组展示
  - 支持按建筑类型筛选和关键词搜索
  - 对缺失图片做前端回退；缺失介绍保持为空白，避免显示占位文案

- 智能问答助手
  - 支持围绕首次参访、新生报到、餐饮、酒店、校史展、VR 和志愿者服务进行快速回答
  - 已在页面顶端服务入口区与主导航中提供入口
...
- 博物馆与 VR 体验
  ...
  - 博物馆概述使用 `概述/图片素材/bg.png` 和 `logo.png` 作为文本背景，不作为实景图直接展示
  - 校史展使用博物馆图片作为背景和分层内容，避免简单横向堆砌
  - 校史展背景图会在多张展览图片之间自动轮换
  - VR 页面截图与二维码分区展示，二维码不再混入实景截图列表

- 餐饮与酒店
  - 使用 `通州校区附近餐饮及酒店/餐饮/` 中的餐厅图片
  - 使用 `通州校区附近餐饮及酒店/酒店/` 中的酒店图片、地址、电话、备注、链接等资料
  - 餐饮、酒店名称和当前已有中文地址、酒店等级、备注支持随界面语言切换
  - 空字段保持为空白，不再显示占位文案

- 多语种界面
  - 页面导航、标题、按钮、统计项、路线摘要、问答提示和常见本地回答纳入语言切换
  - 建筑和博物馆介绍继续优先读取各素材目录中的多语种文本
  - 右上角语言切换按钮使用各语言全名展示，便于国际访客识别
...
脚本会读取 `通州校区建筑/已有建筑实景/` 中有实景资料的建筑，并读取 `通州校区建筑/未有建筑设计图/` 中的未来建筑。15 个大地图坐标维护在脚本的 `POSITION_OVERRIDES` 中，未来建筑会生成 `mapEnabled: false`。餐饮酒店名称和中文自由文本的多语种映射维护在脚本的 `PLACE_NAME_TRANSLATIONS` 与 `FIELD_VALUE_TRANSLATIONS` 中。
...
- 完成 15 个已有实景建筑的地图标记与建筑详情联动
- 完成 15 个大地图建筑坐标校准，坐标参考手绘地图中的建筑名称和红色虚线指示
- 完成中心食堂、先锋剧场、未来传播中心的“敬请期待”索引展示，且不进入大地图索引
- 完成地图标记小图悬浮预览动画
- 完成地图缩放、复位和拖拽浏览
- 完成地图地点和敬请期待地点的搜索与类型筛选
- 完成可接入后端大模型 API 的智能问答助手，并保留无后端时的页面资料回答
- 完成页面静态文案、路线摘要、问答提示、博物馆和建筑介绍的多语种切换
- 完成餐饮、酒店名称及当前已有中文地址、酒店等级、备注的多语种映射展示
- 完成右上角语言切换按钮由英文简写改为各语种全名
- 完成博物馆概述背景化展示、校史展轮换背景展示，以及 VR 截图与二维码分区展示
- 完成面向访客的页面文案优化
- 完成缺图回退，并将缺文本、空字段展示为空白
- 完成导览服务入口仅在页面顶端服务入口区展示，移除地点索引中的“人大徽标”问答助手卡
- 完成公学一楼、艺术楼、学生事务中心、医务中心地图标注坐标调整
- 完成 `site-data.js` 完整素材扫描生成脚本
- 完成 GitHub Pages 与 Vercel 的静态部署配置

## 后续改进方向

- 后续新增建筑或服务点时，继续通过 `scripts/refresh_site_data.py` 统一刷新 `site-data.js`
- 后续补充餐饮、酒店字段资料时，同步补充脚本中的多语种映射，避免非中文页面回退为中文
