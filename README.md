...
- 对于不同语种的界面，我只提供了部分的语言文本映射，请你将对应语种页面的所有内容都在后端翻译为该语种，而不是现在这样无论什么国家的语种里面都有大量的中文
- 新增“未有建筑设计图”文件夹，请将这里面的建筑列入校园地点索引中的“敬请期待”板块，但不支持在大地图上索引，只需展示图片以及介绍
- 对于“已有建筑实景”，我将为你手动校准建筑坐标，我只指出你坐标定位不正确的建筑：中心食堂、先锋剧场、未来传播中心属于未有建筑，不需要在地图上索引；校园运行中心在地图的右下角，你虽然大致位置是正确的，但是还需要精确一下具体位置；学生事务中心的位置在公学二楼的正下方，那里有一个咖啡标志；西南学部楼在京东群学楼的正右方，医务中心（也即之前的健康中心）也同样在公学二楼的下方，那里有一个“AED”的标志；艺术楼在叶澄海楼的右下方，也即西南学部楼的正上方；其他建筑的位置均正确
- 智能问答助手应该也标注咋爱导览索引里面
- 所有板块的标题，比如“智能问答助手。”后面不应该加标点符号，将这种title的标点符号去掉
- 校史展板块的“博物馆概述”中，我在“概述”文件夹中为你提供的图片是logo与background，而不是建筑实景图，你不应该将这两张照片直接在板块中展示，而是应该当作文本的背景
- 右侧校史展的背景首图应该滚动选择，而不是只有一张静态图片
- 请取消没有介绍的餐饮酒店以及建筑的“资料待补充”文本，改为空白文本即可
